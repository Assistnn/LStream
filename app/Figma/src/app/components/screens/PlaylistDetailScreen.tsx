...
import { ChevronDown, Play, GripVertical, X, Clock, SkipBack, SkipForward, RotateCcw, RotateCw, Pause, Repeat, Shuffle, Volume2, FileText, MoreVertical, Download } from 'lucide-react';
import { Playlist, Series, Episode, Chapter, formatDuration, PlaylistItem, Content } from '@/data/mockData';
...
interface PlaylistDetailScreenProps {
  ...
  onPlayItemWithContext?: (playlist: Playlist, playOrder: number[], index: number, isShuffled: boolean, isRepeat: boolean) => void;
        ...
        >
          ...
          {isCurrentlyPlaying && currentTime !== undefined && duration !== undefined ? (
            <div className="h-5 flex items-center gap-2">
              ...
              <span className="text-[10px] font-semibold text-gray-700 dark:text-gray-300 whitespace-nowrap" style={{ minWidth: '65px', textAlign: 'right' }}>
        ...
        <div className="flex flex-col gap-1 items-center flex-shrink-0">
          ...
          {showMenu === index && (
            <>
              ...
              {/* Menu */}
              <div className="absolute right-0 mt-1 w-40 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden z-30" ref={menuRef}>
                <button
                  onClick={() => {
                    onMenuToggle(index);
...
export function PlaylistDetailScreen({
  ...
  const playNextItem = () => {
    ...
    if (nextIndex !== null) {
      ...
      if (onPlayItemWithContext) {
        ...
        const playOrder = isShuffleOn && shuffledOrder.length > 0 
          ? shuffledOrder 
          : playlist.items.map((_, i) => i);
  ...
  const handleNext = () => {
    ...
    if (isShuffleOn && shuffledOrder.length > 0) {
      // Find current position in shuffled order
      const currentShuffledPos = currentPlayingIndex !== null 
        ? shuffledOrder.indexOf(currentPlayingIndex) 
        : -1;
    ...
    <DndProvider backend={HTML5Backend}>
      <div className="fixed top-0 left-0 right-0 bottom-0 bg-white dark:bg-gray-900 z-30 flex flex-col max-w-[460px] mx-auto">
        ...
        <div className="relative bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
          ...
          <div className="relative px-[5%] pt-2 pb-4">
            ...
            <div className="mt-[30px] min-[321px]:mt-[40px]">
              <h1 className="font-bold text-white -ml-3 pl-3 -mr-3 pr-3" style={{ textShadow: '0 2px 6px rgba(0, 0, 0, 0.6)', overflow: 'visible' }}>
              ...
              <div className="h-8 mt-1">
                {playlist.description && (
                  <p className="text-sm text-white/90 line-clamp-2" style={{ textShadow: '0 1px 8px rgba(0, 0, 0, 0.6)' }}>
              ...
              <div className="flex items-center gap-1 text-xs mt-[15px] text-white/60" style={{ textShadow: '0 1px 8px rgba(0, 0, 0, 0.6)' }}>
        ...
        <div className="flex-1 relative">
          <div 
            className="absolute inset-0 overflow-y-auto playlist-scrollbar" 
