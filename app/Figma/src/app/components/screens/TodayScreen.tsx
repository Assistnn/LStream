import { ChevronRight, Headphones, Play, Video } from 'lucide-react'
...
export function TodayScreen({
  ...
  // 今日のおすすめ (最初の3つのシリーズ)
  const todayRecommendations = mockLibraryItems.filter(isSeries).slice(0, 3)
  ...
  return (
    <div className='pb-40 pt-6 dark:bg-gray-900'>
          ...
          <div className='flex gap-4 overflow-x-auto px-[5%] pb-2 scrollbar-hide snap-x snap-mandatory'>
            {todayRecommendations.map((series) => (
              <div key={series.id} className='flex-shrink-0 w-[85%] snap-center'>
                <SeriesCard series={series} variant='large' onTap={onSeriesTap} />
              </div>
            ))}
          </div>
        </div>
      ...
      {continueItems.length > 0 && (
        <div className='mb-8'>
          <h2 className='text-lg font-bold text-gray-900 dark:text-white mb-4 px-[5%]'>前回の続き</h2>
          <div className='flex gap-3 overflow-x-auto px-[5%] pb-2 scrollbar-hide'>
            {continueItems.map((item) =>
              isSeries(item) ? (
                <SeriesCard key={item.id} series={item} variant='small' onTap={onSeriesTap} />
              ) : (
                <ContentCard key={item.id} content={item} variant='small' onPlay={onPlayContent} />
              ),
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <div className='bg-white dark:bg-gray-800 border-t border-b border-gray-200 dark:border-gray-700'>
            {Object.keys(groupedRecentHistory).map((dateKey) => (
              <div key={dateKey}>
                <h3 className='text-sm font-bold text-gray-900 dark:text-white px-[5%] py-2'>{dateKey}</h3>
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-3 px-[5%] py-3 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${
                        index !== groupedRecentHistory[dateKey].length - 1
