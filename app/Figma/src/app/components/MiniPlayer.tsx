import { Heart, List, Pause, Play, X } from 'lucide-react'

import type { Content } from '@/data/mockData'

interface MiniPlayerProps {
  content:
    | (Content & { subtitle?: string; episodeNumber?: number; chapterNumber?: number; isFavorite?: boolean })
    | null
  isPlaying: boolean
  onTogglePlay: () => void
  onTap: () => void
...
export function MiniPlayer({
  ...
  return (
    <div
      className='fixed bottom-16 left-0 right-0 bg-gray-100 dark:bg-gray-750 border-t border-gray-300 dark:border-gray-600 shadow-lg z-40'
      style={{ backgroundColor: 'var(--mini-player-bg, rgb(243 244 246))' }}
    >
      <style>{`
      ...
      `}</style>
      <div className='flex items-center gap-0 cursor-pointer' onClick={onTap}>
        {/* 16:9 Thumbnail - No left padding */}
        {content.thumbnail ? (
          <img src={content.thumbnail} alt={content.title} className='w-[90px] h-[56px] object-cover flex-shrink-0' />
        ) : (
        ...
        <div className='flex-1 min-w-0 px-3 flex items-center gap-2'>
          <div className='flex-1 min-w-0'>
            <h4 className='text-sm font-semibold text-gray-900 dark:text-white truncate'>{content.title}</h4>
          ...
          {onToggleFavorite && (
            ...
            >
              <Heart
                className={`w-4 h-4 ${
                  content.isFavorite ? 'fill-blue-500 text-blue-500' : 'text-gray-500 dark:text-gray-400'
                }`}
              />
            </button>
          )}
