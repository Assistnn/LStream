...
import { DownloadScreen, DownloadItem } from '@/app/components/screens/DownloadScreen';
...
import { AccountScreen, ServiceAccount } from '@/app/components/screens/AccountScreen';
...
import { Content, Series, Episode, Chapter, mockLibraryItems, Playlist, PlaylistItem, AlarmSettings } from '@/data/mockData';
...
export default function App() {
  const [activeTab, setActiveTab] = useState<'today' | 'library' | 'favorites' | 'playlist' | 'history' | 'settings'>('today');
  ...
  const [pendingPlaylistItem, setPendingPlaylistItem] = useState<{ series: Series; episode: Episode; chapter?: Chapter } | null>(null);
  ...
  const activeServiceAccount = serviceAccounts.find(
    acc => acc.id === activeServiceAccountId
  ) || null;
  ...
  const handleNextEpisode = () => {
    if (playingItem?.type === 'episode') {
      ...
      const currentIndex = sortedEpisodes.findIndex(
        (ep) => ep.id === playingItem.episode.id
      );
  ...
  const handleNextChapter = () => {
    if (playingItem?.type === 'episode' && playingItem.chapter) {
      ...
      const currentChapterIndex = currentEpisode.chapters?.findIndex(
        (ch) => ch.id === playingItem.chapter?.id
      ) ?? -1;
  ...
  const handleUpdatePlaylist = (id: string, name: string, description: string, alarm?: AlarmSettings, coverImage?: string) => {
    setPlaylists(playlists.map(p => 
      p.id === id 
        ? { ...p, name, description, alarm, coverImage }
        : p
    ));
  ...
  const currentMiniPlayerContent: Content | null = playingItem
    ? playingItem.type === 'content'
      ...
      : {
          ...playingItem.episode,
          title: playingItem.chapter 
            ? playingItem.chapter.title 
            : playingItem.episode.title,
        ...
        {showPlayer && playingItem && (() => {
          ...
          return (
            <EpisodePlayer
              ...
              hasNextEpisode={playlistPlaybackMode ? (playlistPlaybackMode.isRepeatOn || playlistPlaybackMode.currentIndex < playlistPlaybackMode.order.length - 1) : (() => {
                const sortedEpisodes = getSortedEpisodes(playingItem.series);
                const currentIndex = sortedEpisodes.findIndex(
                  (ep) => ep.id === playingItem.episode.id
                );
              ...
              hasPrevEpisode={playlistPlaybackMode ? (playlistPlaybackMode.currentIndex > 0) : (() => {
                const sortedEpisodes = getSortedEpisodes(playingItem.series);
                const currentIndex = sortedEpisodes.findIndex(
                  (ep) => ep.id === playingItem.episode.id
                );
              ...
              playlistMode={playlistPlaybackMode ? {
